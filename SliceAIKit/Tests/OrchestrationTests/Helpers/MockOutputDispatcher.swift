...
/// - **默认 `.delivered`**：测试方便；可通过 `outcomeOverride` 闭包按 `PresentationMode`
...
final actor MockOutputDispatcher: OutputDispatcherProtocol {
    ...
    private(set) var calls: [(chunk: String, mode: PresentationMode, invocationId: UUID)] = []

    /// 注入特定模式应返回的 `DispatchOutcome`；nil 时默认 `.delivered`
    /// `@Sendable` 是 actor field 存储跨边界闭包的硬性要求
    private var outcomeOverride: (@Sendable (PresentationMode) -> DispatchOutcome)?

    /// 构造 mock；可选注入 `outcomeOverride` 控制返回值
    init(outcomeOverride: (@Sendable (PresentationMode) -> DispatchOutcome)? = nil) {
    ...
    func setOutcomeOverride(_ override: @escaping @Sendable (PresentationMode) -> DispatchOutcome) {
