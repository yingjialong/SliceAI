...
/// 把 `ProviderSelection`（fixed / capability / cascade）解析到具体 `V2Provider`。
///
/// M2 只实现 `.fixed`：按 providerId 在 `V2Configuration.providers` 里查找，命中则返回 V2Provider，
...
///
/// `selection.modelId` 在 M2 阶段**不**被本协议消费——返回的 `V2Provider` 始终是配置里的原始定义，
...
public actor DefaultProviderResolver: ProviderResolverProtocol {
    ...
    /// 按需获取最新 V2Configuration 的闭包；测试用 `{ stubConfig }`，生产用 `{ await store.current() }`
    private let configurationProvider: @Sendable () async throws -> V2Configuration

    // MARK: - Init

    /// 构造默认 resolver
    /// - Parameter configurationProvider: 按需获取最新 V2Configuration 的闭包；测试用 `{ stubConfig }`，
    ///   生产用 `{ await store.current() }`（M3 装配）
    public init(configurationProvider: @Sendable @escaping () async throws -> V2Configuration) {
        self.configurationProvider = configurationProvider
    }

    // MARK: - ProviderResolverProtocol

    /// 解析 ProviderSelection 到具体 V2Provider
    ///
    /// M2 范围：`.fixed` 完整实现；`.capability` / `.cascade` 抛 notImplemented
    public func resolve(_ selection: ProviderSelection) async throws -> V2Provider {
        switch selection {
