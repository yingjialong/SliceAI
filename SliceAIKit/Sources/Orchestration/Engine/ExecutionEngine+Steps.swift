...
extension ExecutionEngine {
    ...
    func runProviderResolution(
        ...
        do {
            ...
            if Task.isCancelled { return nil }
            let capabilityList = requires.map(\.rawValue).sorted().joined(separator: ", ")
            await finishFailure(
                error: .configuration(.validationFailed(
                    "No provider supports required capabilities: \(capabilityList)"
                )),
                effective: context.effective, context: context
            )
            return nil
