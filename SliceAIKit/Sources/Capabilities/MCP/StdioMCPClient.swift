...
extension StdioMCPClient {
    ...
    private func startProcess(for descriptor: MCPDescriptor) throws -> StdioMCPProcessSession {
        ...
        process.environment = environment(for: descriptor)
    ...
    /// 合并系统环境变量和 descriptor 指定的 env 覆盖项
    /// - Parameter descriptor: 当前 stdio server descriptor
    /// - Returns: 可直接传给 `Process.environment` 的环境变量字典
    private func environment(for descriptor: MCPDescriptor) -> [String: String] {
