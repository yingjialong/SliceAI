...
| MCP / Skill | v0.2+ 扩展方向 | **v2.0 的定位必需；MCP 在 Phase 1 落地，Skill 在 Phase 2 落地** |
...
- "MCP / Skill 在 v0.2+" → 明确 MCP 是 Phase 1 必须落地，Skill 是 Phase 2 首要能力
...
| **1** | MCP + Context 主干 | **Freeze** | 20–30 | MCP 支持 / 5 个核心 ContextProvider / Per-Tool Hotkey / 基础自定义 Agent Tool | MCPClient（stdio + Streamable HTTP）+ MCPServersPage + AgentExecutor + Agent Tool 编辑器 + `web-search-summarize` 首个真 Agent Tool |
...
- 用户可以在 Settings 里加 MCP server，并在 Tool 里配置哪些 MCP tool 可用。
- 用户可以在 Settings 里创建和编辑基础 Agent Tool，配置 prompt、provider、MCP allowlist 和调用策略；内置 `web-search-summarize` 只是示例，不应是唯一可用 Agent。
- Per-Tool Hotkey 生效。
...
| 1.11 | 基础 Agent Tool 编辑器 | 用户可创建 Agent、编辑 system / initial prompt、选择 tool-calling provider、配置 MCP allowlist |
| 1.12 | Agent tool-call policy | 独立于 `maxSteps` 的总调用数 / 单轮调用数 / 每 tool 调用数 / 重复参数 / 限流停止策略 |
...
- [x] 用户可从 Settings 创建一个基础 Agent Tool，配置 MCP allowlist 和调用策略；Command Palette / hotkey App E2E 仍在 Task 17 回归。
- [x] AgentExecutor 使用独立 tool-call policy 控制 MCP 调用次数、重复参数和限流停止；`maxSteps` 只表示 LLM ReAct 轮数。
...
| **成本失控（agent loop 死循环 / 单轮多 tool call 打爆 MCP 限流）** | 中 | 高 | `maxSteps` 限 LLM 轮数；Agent tool-call policy 独立限制总调用数 / 单轮调用数 / 每 tool 调用数 / 重复参数 / 限流停止；per-tool budget + 全局 budget 负责成本硬停 |
