...
- [x] 运行 `swift test --filter CapabilitiesTests.ContextProviderTests` 确认红灯。
...
5. `FileReadContextProvider` 使用 `request.args["path"]`，先通过 `PathSandbox.normalize(_:role: .read)` 规范化并校验，再用 UTF-8 读取文本；读取前后各检查一次取消。权限推导为 `args["path"]` 对应的 `[.fileRead(path:)]`，调试日志只输出文本长度，不输出路径或文件内容。
...
- `swift test --filter CapabilitiesTests.ContextProviderTests`：编译失败，错误为 `cannot find 'SelectionContextProvider' in scope`、`cannot find 'AppWindowTitleContextProvider' in scope`、`cannot find 'AppURLContextProvider' in scope`、`cannot find 'ClipboardCurrentContextProvider' in scope`、`cannot find 'FileReadContextProvider' in scope`，符合 provider 类型缺失预期。

最终验证：

- `swift test --filter CapabilitiesTests.ContextProviderTests`：7 tests passed。
- `swift test --filter OrchestrationTests.ContextCollectorTests`：10 tests passed。
- `swift test --filter OrchestrationTests.PermissionGraphTests`：25 tests passed。
- `git diff --check`：通过。

## Self-review

- 文件读取先走 `PathSandbox.normalize` 再读文件，硬禁止路径即使被用户 allowlist 覆盖也会拒绝。
- 触及 IO 的剪贴板和文件 provider 均在 IO 边界前后检查取消；纯 seed/app provider 不需要额外取消点。
