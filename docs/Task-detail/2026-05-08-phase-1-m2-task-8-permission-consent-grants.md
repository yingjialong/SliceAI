...
- `PersistentPermissionGrantStore` 读侧新增 schema、scope、permission 一致性和 provenanceTag 校验；损坏或手改的 grant 文件 fail-closed。
- session / persistent store 错误类型分别命名为 `SessionPermissionGrantStoreError` 与 `PersistentPermissionGrantStoreError`，避免 App/Settings 同时 import Orchestration 和 Capabilities 后产生 API 歧义。
...
- code-quality review follow-up 覆盖：默认路径断言、磁盘中伪造 session grant fail-closed、schemaVersion 不匹配 fail-closed、grant.permission 不一致 fail-closed。
...
- `cd SliceAIKit && swift test --filter CapabilitiesTests.PersistentPermissionGrantStoreTests`：通过（7 tests）。
- `cd SliceAIKit && swift test`：通过（674 tests）。
