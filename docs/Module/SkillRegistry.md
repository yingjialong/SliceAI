...
SkillRegistry 负责暴露首方内置 skill、扫描用户配置的本地 skill roots，解析 Claude / Codex 风格 `SKILL.md`，生成可供 Settings 和 AgentExecutor 使用的 registry snapshot。

## MVP 边界

- 读取 `SKILL.md`，并按需只读加载已索引的 `references/` 与文本型 `assets/` supporting files。
- 默认提供首方内置 `english-tutor` skill，供 English Tutor 默认工具绑定。
- 不执行 `scripts/`。
- 不读取 `scripts/`、二进制 assets、`agents/openai.yaml` 或未索引路径。
...
- 通过 `sliceai_load_skill_resource` 在已加载 bound skill 范围内读取 supporting file。
...
2. `LocalSkillRegistry.snapshot()` 先加入内置首方 skill，再按 source order 解析候选 skill，合并 frontmatter、用户 override、大小限制和 duplicate name 规则。
...
6. 模型需要 supporting file 时，必须先加载同名 skill，再调用 `sliceai_load_skill_resource` 读取 metadata 中列出的相对路径。
7. ResultPanel 继续使用现有 `toolCallProposed` / `toolCallApproved` / `toolCallResult` / `toolCallError` 生命周期展示加载过程。
...
- supporting files 已支持 `references/` 与文本型 `assets/` 只读加载；后续需要继续评估二进制 assets、资源大小策略和 UI 可观测性。
