...
- `.prompt`：真实执行路径，包含 system/user prompt、provider selection、temperature、变量表。
...
核心源码位于 `SliceAIKit/Sources/SliceCore/`。测试位于 `SliceAIKit/Tests/SliceCoreTests/`，覆盖配置迁移、canonical JSON、权限编码、SelectionPayload 到 ExecutionSeed 映射、MCP JSON contract、Skill 配置模型、错误脱敏和默认配置生成。
